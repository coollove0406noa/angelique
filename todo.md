...
- [ ] 現在の認証フロー・リダイレクト原因を調査
- [ ] /session/:token をManusログイン不要でアクセス可能に修正
- [ ] App.tsxのルーティングでセッションページを認証ガード外に移動
- [ ] 動作確認（未ログイン状態でセッションURLに直接アクセス）

## バグ修正・UI改善（11項目）

- [ ] 1. Socket.ioでセッション開始時にお客様画面を自動切替（waiting→chat）
- [ ] 2. 短いメッセージの折りたたみを完全に無効化
- [ ] 3. タイマーを画面上部に固定表示（スクロールしても常に見える）
- [ ] 4. 延長URLをtarget="_blank"で別タブ表示に変更
- [ ] 5. セッション終了時にお客様画面に終了メッセージを自動表示
- [ ] 6. 管理者画面の「お客様URL」に「このURLをお客様に送ってください」説明文を追加
- [ ] 7. タロットカード表示時間を5秒→15秒、フェード時間を2秒に変更
- [ ] 8. 延長確認バーを画面上部に固定表示・チャット内URLテキストを削除
- [ ] 9. タイマーを管理者「開始」ボタン押下時刻からの実経過時間基準に変更
- [ ] 10. 延長時間の二重加算バグを修正（10分延長→10分のみ追加）
- [ ] 11. 延長選択肢を「10分延長」「30分延長」の2つのみに変更（20分削除）

## 追加修正（延長URL4種類・お客様終了ボタン）

- [ ] AdminSettings.tsxの延長URL設定を4種類に拡張（チャット10分・チャット30分・音声10分・音声30分）
- [ ] AdminSession.tsxの延長URL送信をsessionTypeに応じて自動選択
- [ ] ClientSession.tsxにセッション終了ボタンを追加（確認ダイアログ付き）
- [ ] Socket.ioでお客様終了イベントを管理者に通知
- [ ] 管理者画面に「お客様がセッションを終了しました」通知を表示
- [ ] お客様画面に終了後メッセージを表示
